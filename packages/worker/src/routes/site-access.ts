...
accessRoutes.patch('/sites/:id/access', async (c) => {
  ...
  const current = await getSiteAccess(c.env, site.id);
  const nextVersion = (current?.policy_version || 0) + 1;
  ...
  if (payload.mode === 'link') {
    ...
    await c.env.DB.prepare(`
      ...
      ON CONFLICT(site_id) DO UPDATE SET
        ...
        policy_version = excluded.policy_version,
        updated_at = datetime('now')
    `).bind(site.id, password.hash, password.salt, nextVersion).run();
  }
...
accessRoutes.post('/.vanish/access', async (c) => {
  ...
  if (contentType.includes('application/json')) {
    const payload = await c.req.json<{ site?: string; password?: string; return?: string }>().catch(() => null);
    siteId = payload?.site || '';
    password = payload?.password || '';
    returnPath = sanitizeReturnPath(payload?.return);
  } else {
  ...
  if (!valid) {
    await c.env.DB.prepare('INSERT INTO rate_limits (identifier, action) VALUES (?, ?)')
