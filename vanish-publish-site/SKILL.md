...
3. Enumerate the site manifest, including dotfiles, before upload:

```bash
find <folder> -type f | sort
```

4. If the manifest or filenames suggest sensitive content, stop and ask before uploading.
5. Run the default JSON command:

```bash
vanish site <folder> --root <root-file> --json --no-clipboard
```
